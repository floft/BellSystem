...
namespace DateTime
{
	...
	date::date(const string& dd)
		:y(0), m(0), d(0)
	{
		if (dd.length() != 8) throw Invalid();
		
		y = string_to_int(dd.substr(0,4));
		m = string_to_int(dd.substr(4,2));
		d = string_to_int(dd.substr(6,2));

		if (!valid(y,m,d))
			throw Invalid();
	}

	bool date::valid(const int& yy, const int& mm, const int& dd)
	{
	...
	bool date::leapyear(const int& yy)
	{
	...
	int date::days_in_month(const int& yy, const int& mm)
	{
		int days = 31;
	...
	time::time(const string& t)
		:h(0), m(0)
	{
		vector<string> parts = Split::split(t, ":");
	...
	bool time::valid(const int& hh, const int& mm)
	{
